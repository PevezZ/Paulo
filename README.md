...
2 Conecte os servomotores aos pinos digitais do Arduino conforme definido no código (ARM1_PIN=10, ARM2_PIN=11, BASE_PIN=9).
3 Faça o upload do sketch para o seu Arduino.
4 Anote a porta serial à qual o Arduino está conectado (ex: /dev/ttyUSB0 no Linux, COM3 no Windows).
 
3. Configuração da Câmera (DroidCam)
5 Instale o aplicativo DroidCam em seu smartphone e o cliente DroidCam em seu computador.
6 Configure a DroidCam para transmitir vídeo via IP e anote o endereço IP e a porta (ex: http://YOUR_DROIDCAM_IP_ADDRESS:PORT/video).
7 Importante: Substitua "http://YOUR_DROIDCAM_IP_ADDRESS:PORT/video" no código Python pela URL real da sua DroidCam.
